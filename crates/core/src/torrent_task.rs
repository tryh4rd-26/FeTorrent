...
use crate::peer::session::{PeerCommand, PeerEvent, MAX_INFLIGHT_REQUESTS};
...
pub struct TorrentTaskParams {
    pub tracker_infos: Vec<TrackerInfo>,
    pub torrent: Option<TorrentFile>,
    pub storage: Option<Storage>,
    pub save_path: std::path::PathBuf,
    pub command_rx: mpsc::Receiver<TorrentCommand>,
    pub update_tx: mpsc::Sender<TorrentUpdate>,
}
...
impl TorrentTask {
    pub fn new(
        ...
        info_hash: [u8; 20],
        peer_id: [u8; 20],
        params: TorrentTaskParams,
    ) -> Self {
        let TorrentTaskParams {
            tracker_infos,
            torrent,
            storage,
            save_path,
        ...
        let piece_manager = torrent
            .as_ref()
            .map(|t| PieceManager::new(t.piece_length, t.total_size, t.pieces.clone()));
        let peer_manager = PeerManager::new(info_hash, peer_id, 300, event_tx.clone());

        let left = if let Some(t) = &torrent {
            t.total_size
        } else {
            0
        };
        ...
        loop {
            tokio::select! {
                ...
                Some(cmd) = self.command_rx.recv() => {
                    if let TorrentCommand::Stop = cmd { break }
                }
        ...
        match event {
            PeerEvent::NewPeer { addr, tx } => {
                ...
                m.insert(
                    ...
                    BValue::Int(UT_METADATA_EXT_ID as i64),
                );
                        ...
                        if let Some(BValue::Int(id)) = m.get(b"ut_metadata".as_ref()) {
                            ...
                            if let Some(BValue::Int(size)) = d.get(b"metadata_size".as_ref()) {
                                ...
                                if self.metadata_size.is_none() && size > 0 {
                                    ...
                                    let num_pieces = size.div_ceil(16384);
                                    info!(
                                        "Metadata size discovered: {} bytes ({} pieces)",
                                        size, num_pieces
                                    );
                                }
                            }
    ...
    async fn pick_and_request(&mut self, addr: SocketAddr) {
        ...
        for (&idx, active) in &self.active_pieces {
            if let Some(bf) = self.peer_bitfields.get(&addr) {
                ...
                let bit_idx = 7 - (idx % 8);
                if byte_idx < bf.len()
                    && (bf[byte_idx] & (1 << bit_idx)) != 0
                    && active.blocks_received < active.total_blocks
                {
        ...
        if let Some(idx) = piece_index {
            if let (Some(active), Some(pm)) = (self.active_pieces.get(&idx), &self.piece_manager) {
                if let Some(tx) = self.active_peers.get(&addr) {
                    let missing = active.get_missing_blocks(pm, MAX_INFLIGHT_REQUESTS);
        ...
        if let Ok(BValue::Dict(d)) = decoder.decode() {
            ...
            if msg_type == Some(1) {
                ...
                if let Some(p_idx) = piece {
                    ...
                    if self.metadata_pieces.contains(&p_idx) {
                        return;
                    }
                    ...
                    if let Some(size) = self.metadata_size {
                        ...
                        if offset + data.len() <= size {
                            ...
                            info!(
                                "Received metadata piece {}/{} from {} (len {})",
                                p_idx + 1,
                                self.metadata_size.map(|s| s.div_ceil(16384)).unwrap_or(0),
                                addr,
                                data.len()
                            );
                        }
                    ...
                    if let Some(size) = self.metadata_size {
                        ...
                        if self.metadata_pieces.len() == num_pieces {
                            ...
                            if let Ok(t) = TorrentFile::from_info_dict(&self.metadata_buf) {
