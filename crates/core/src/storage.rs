...
use crate::torrent::{TorrentFileEntry, TorrentFile};
...
impl Storage {
    pub fn new(base_path: impl Into<PathBuf>, torrent: &TorrentFile) -> Result<Self> {
        ...
        std::fs::create_dir_all(&base_path).map_err(|e| CoreError::StorageIo(format!("Create dir: {}", e)))?;
        ...
        for entry in &torrent.files {
            ...
            if let Some(parent) = path.parent() {
                std::fs::create_dir_all(parent).map_err(|e| CoreError::StorageIo(format!("Parent dir: {}", e)))?;
            }
            ...
            file.set_len(entry.size).map_err(|e| CoreError::StorageIo(format!("Set length: {}", e)))?;
    ...
    pub fn read_block(&mut self, absolute_offset: u64, mut length: u32, buf: &mut [u8]) -> Result<()> {
        ...
        for open_file in &mut self.files {
            ...
            if current_offset >= file_start && current_offset < file_end {
                ...
                open_file.file.read_exact(&mut buf[buf_pos..(buf_pos + bytes_to_read as usize)])
    ...
    pub fn read_piece(&mut self, piece_index: u32, piece_length: u32, data: &mut std::vec::Vec<u8>) -> Result<()> {
        let offset = piece_index as u64 * piece_length as u64;
        let len = self.total_size.saturating_sub(offset).min(piece_length as u64) as usize;
        data.resize(len, 0);
